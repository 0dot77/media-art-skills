...
### 3. Manual Generation (프로젝트 타입별 분기)

TECH-SPEC.md를 기반으로 다음 섹션을 생성한다.

**프로젝트 타입별 설치 중점:**

| 프로젝트 타입 | 설치 핵심 단계 | 캘리브레이션 중점 |
|-------------|-------------|---------------|
| 프로젝션 매핑 | 마운트 설치 → 정렬 → 블렌딩 → 워핑 → 색보정 | 프로젝터 간 밝기/색 매칭, 소프트엣지 조정 |
| LED 디스플레이 | 구조물 → 캐비닛 조립 → 전원 연결 → 프로세서 설정 | 패널 간 밝기 균일도, 데드 픽셀 체크 |
| 인터랙티브 | 디스플레이 설치 → 센서 설치 → 좌표 매핑 → 인터랙션 테스트 | 센서-디스플레이 좌표 정합, 레이턴시 측정 |
| 키네틱 | 구조물/리깅 → 기구부 조립 → 모터 테스트 → 안전 테스트 | 동작 범위 확인, 비상 정지 테스트, 소음 측정 |
| 이머시브 | 다면 설치 → 각 면 정렬 → 이음새 블렌딩 → 음향 → 몰입도 테스트 | 다면 이음새 최소화, 관객 위치별 왜곡 보정 |
...
| 문서 | 읽는 필드 | 용도 |
|------|----------|------|
| `TECH-SPEC.md` | `## 장비 목록`, `## 네트워크 구성`, `## 전원 순서` | 설치 매뉴얼 핵심 소스 |
| `SITE-SURVEY.md` | `## 접근`, `## Red Flags` | 반입 경로, 현장 제약 반영 |
| `DISPLAY-CALC.md` | 디스플레이 배치, 해상도 설정 | 캘리브레이션 절차 |
